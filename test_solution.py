...
def test_empty_result_when_dataframe_columns_is_invalid(): 
    df = pd.DataFrame(
        [[1, 2]] * 3,
        columns = ["label-one", "label-two"],
    )
...
def test_original_datagrame_is_not_modified():
    df = pd.DataFrame(
        [[2, 3]], 
...
def test_complex_expressions():
    ...
    df_result = add_virtual_column(
        df, 
        "label_one - label_two + label_three * label_two",
        "result_column",
    )
